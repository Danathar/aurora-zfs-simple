...
| `test-quality-docs.sh`      | `docs/quality.md`, `docs/metrics.md` and `docs/review-rubric.md`: the badge and gate tables against `.github/workflows/build.yml`, `.github/workflows/status-badges.yml`, the `Containerfile` and `ci/write-badges.sh`; the metrics commands parsed, their `jq` filters compiled and every `gh` call in them naming this repository; each dated snapshot under `docs/metrics/` linked from `metrics.md`, titled with its file's date, its commands parsed, their filters compiled, every `gh` call naming this repository and every run or pull request listing pinned to a fixed scope; and the rubric's checks, incident comments, shell bar, evidence table and signing claim against the code each one names |
...
| `test-readme.sh`            | `README.md`: every claim in it that names something here and no other test reads, recomputed rather than restated — the post-check list against `main()` in `build_files/post-check.sh`, each bullet tied by a manifest to the check that implements it and a line of that check's body, with a check no bullet accounts for and a bullet no check accounts for both failing; the script's own execution-order header against `main()`; "What It Uses" against the `Containerfile`'s pulled images and the workflow's `CHUNKAH_IMAGE` in both directions; the quoted `ARG` lines and the Fedora guard; the kernel-pin example against the akmods stages it replaces, one shared tag of the template's shape; the schedule sentence against `build.yml`'s cron; the badge counts and build workflow name; the step name and Chunkah tag the build section quotes; and the Repository Layout block listing every tracked entry of `.github/workflows/`, `build_files/`, `ci/` and `docs/` |
| `test-contributing.sh`      | `CONTRIBUTING.md`: every claim in it that names something here, recomputed rather than restated — the tool list against `run-tests.sh`'s preflight loop and its two commands against the runner's usage lines, the PyYAML package and `WORKFLOW_PYTHON` against the tests and the `Shell tests` job, the shellcheck paragraph against `test-shell-syntax.sh`, the unreachable build scripts against `build_files/` both ways, the quoted guard verbatim, the reached `check_*` stages against `test-post-check-checks.sh` both ways with their count words, every publishing step gated off pull requests, the gotcha's `paths-ignore` list, the `set` rule against every tracked shell script's first `set` line with only the exceptions the page names, the `tests/lib/` exemption, and the `ai-fix.yml` triggers, label and bot refusal |
